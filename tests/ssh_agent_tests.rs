...
#[tokio::test]
async fn test_windows_ssh_agent_creation() -> Result<(), Box<dyn Error>> {
...
#[tokio::test]
async fn test_sign_with_nonexistent_key() -> Result<(), Box<dyn Error>> {
    let mut ssh_agent = create_test_agent()?;

    let test_data = b"Test signing";
    let fake_public_key = vec![0, 1, 2, 3];

    let result = ssh_agent.sign_data(&fake_public_key, test_data).await;
